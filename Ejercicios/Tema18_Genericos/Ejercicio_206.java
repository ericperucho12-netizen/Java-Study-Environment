package Ejercicios.Tema18_Genericos;

/**
 * ---------------------------------------------------------
 * EJERCICIO 206: CLASES GENÉRICAS
 * ---------------------------------------------------------
 * Conceptos: <T>, Clases Genéricas, Type Safety.
 * 
 * Enunciado:
 * Instancia una clase genérica utilizando el tipo Double 
 * para guardar y extraer un número decimal. (Práctica #206).
 */
 
class Contenedor_206<T> {
    private T valor;
    public void setValor(T v) { this.valor = v; }
    public T getValor() { return this.valor; }
}

public class Ejercicio_206 {
    public static void main(String[] args) {
        System.out.println("=== USO DE GENÉRICOS #206 ===");
        
        Contenedor_206<Double> miContenedor = new Contenedor_206<>();
        miContenedor.setValor(3.1415 + (206 - 200));
        
        System.out.println("Valor almacenado: " + miContenedor.getValor());
    }
}